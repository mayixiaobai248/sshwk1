...
IMAGE_OPTIONAL_HEADER = ['Magic','MajorLinkerVersion',
          ...
          'MajorSubsystemVersion','MinorSubsystemVersion','Reserved1','SizeOfImage','SizeOfHeaders',
...
def get_PEHeader_features(path):
    ...
    PEHeader_features = [0 for i in range(len(IMAGE_OPTIONAL_HEADER))]
    try:
      pe = pefile.PE(path)
    except:
        pass
    ...
    if 'NT_HEADERS' in list:
        temp = dir(pe.NT_HEADERS)
        if 'OPTIONAL_HEADER' in temp:
            for i in range(len(IMAGE_OPTIONAL_HEADER)):
                PEHeader_features[i] = getattr(pe.NT_HEADERS.OPTIONAL_HEADER, IMAGE_OPTIONAL_HEADER[i])
        else:
